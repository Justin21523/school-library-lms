/**
 * MARC21 Field Dictionary（Web side）
 *
 * 目標：
 * - 你要「常用 MARC21 欄位」有完整的下拉選單、指標（ind1/ind2）可選、子欄位 code 可選
 * - 並且要有「欄位級驗證」：避免前端把明顯不合法的形狀送進 API
 *
 * 注意（重要取捨）：
 * - 完整 MARC21（BIB）規格非常大（每個 tag 的可用子欄位/指標意義/可重複性都不相同）。
 * - 本檔先落地「常用欄位字典」：覆蓋我們在 LMS 常見的書目編目欄位（020/245/264/300/5XX/6XX/7XX/8XX/856…）。
 * - 未列入字典的 tag：前端仍允許手動輸入，但會降級為「通用編輯/通用驗證」。
 *
 * 你後續若要「更完整」：
 * - 只要在 `MARC21_BIB_FIELDS` 增加 spec，即可同時獲得：
 *   - tag dropdown 顯示
 *   - 指標/子欄位下拉
 *   - per-tag 驗證
 */

import type { MarcDataField, MarcField, MarcSubfield } from './api';
...
export function createEmptyFieldFromSpec(tag: string): MarcField {
  ...
  if (spec.kind === 'control') {
    // 固定欄位（006/008 等）會有 exact_length；v1 先用空白補滿（符合 MARC 語意）
    // - 這樣做的好處是：新增欄位後立刻就是「合法長度」，館員只要把位置填上去即可。
    const exact = spec.value.exact_length;
    if (typeof exact === 'number' && exact > 0) return { tag: spec.tag, value: ' '.repeat(exact) };
    return { tag: spec.tag, value: '' };
  }
