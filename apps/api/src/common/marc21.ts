/**
 * MARC21 Field Dictionary（API side）
 *
 * 目的：
 * - 提供「常用 MARC21（BIB）」欄位字典，讓後端能對 `marc_extras` 做欄位級驗證：
 *   - indicators 值是否合法
 *   - 子欄位 code 是否合理（可設定 strict / allow-other）
 *   - 子欄位值的最小格式（URL/年份/ISBN…）
 *   - repeatable=false 的欄位不可重複（array-level）
 *
 * 設計取捨：
...
import type { MarcDataField, MarcField, MarcSubfield } from './marc';
