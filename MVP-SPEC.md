...
## 4) 主要流程（MVP）
1. **建置**：建立分館/位置 → 匯入使用者 → 建立借閱政策（天數/上限/續借/逾期規則；含 `overdue_block_days`）
...
- 預約上限：3 本
- 到館保留：3 天
- 逾期：到期隔天列入逾期名單；逾期達 7 天「停權新增借閱」（對應 policy 欄位 `overdue_block_days=7`；仍可還書/取消預約）
...
- 預約上限：5 本
- 到館保留：5 天
- 逾期：到期隔天列入逾期名單；逾期達 14 天「停權新增借閱」（對應 policy 欄位 `overdue_block_days=14`）
