...
-- v1.1 migration（舊 DB 可能沒有 is_active 欄位）
-- - 若你之前已經建立過 circulation_policies，但 schema 仍停留在 v1.0（沒有 is_active）
--   後續建立 partial unique index / seed demo 都會失敗。
...
--
-- 注意：PostgreSQL 要求「index expression」使用的函數必須是 IMMUTABLE。
-- - array_to_string() 在某些版本/設定下會被標示為非 IMMUTABLE，導致建 index 直接失敗
-- - 我們用一個 wrapper function 明確宣告為 IMMUTABLE（輸入固定 → 輸出固定），再用它建 index
--
-- 好處：
-- - API 端的 ILIKE/similarity 直接用同一個 function，確保能吃到 index
-- - schema 可重複套用（CREATE OR REPLACE + CREATE INDEX IF NOT EXISTS）
CREATE OR REPLACE FUNCTION authority_terms_search_text(preferred_label text, variant_labels text[])
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT preferred_label || ' ' || COALESCE(array_to_string(variant_labels, ' '), '');
$$;

CREATE INDEX IF NOT EXISTS authority_terms_search_trgm
  ON authority_terms USING gin (authority_terms_search_text(preferred_label, variant_labels) gin_trgm_ops);
...
-- ============================================================
-- Multi-tenant hardening（第二道防線）：Row Level Security (RLS)
-- ============================================================
--
-- 目標：
-- - 即使程式碼某天「忘了加 organization_id 條件」，DB 仍能阻止跨校資料被讀到/改到
--
-- 設計：
-- - 以 Postgres session variable `app.org_id` 作為「目前請求的租戶」上下文
-- - API 會在每次 DB 操作前設定 `app.org_id`（見 DbService）
-- - policy 用 `organization_id = app_current_org_id()` 做隔離
--
-- 重要取捨（務必讀）：
-- - 我們使用 `FORCE ROW LEVEL SECURITY`，避免「同一 DB user 同時是 table owner」時 RLS 被繞過
--   （MVP 本機/docker 常見就是這種配置）
-- - 代價：任何對 org-scoped tables 的 SELECT/INSERT/UPDATE 都必須先 set app.org_id
--   → 這就是我們把「設定 org context」集中在 DbService 的原因

-- app_current_org_id：把 session variable 轉成 uuid（未設定或空字串時回 NULL）
...
-- 1) org-scoped tables：直接用 organization_id 做隔離
--    - USING：控制 SELECT/DELETE 可見性
--    - WITH CHECK：控制 INSERT/UPDATE 是否允許寫入
...
ALTER TABLE bibliographic_records FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_records_org_isolation ON bibliographic_records;
CREATE POLICY bibliographic_records_org_isolation ON bibliographic_records
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE bibliographic_identifiers ENABLE ROW LEVEL SECURITY;
ALTER TABLE bibliographic_identifiers FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_identifiers_org_isolation ON bibliographic_identifiers;
CREATE POLICY bibliographic_identifiers_org_isolation ON bibliographic_identifiers
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE item_copies ENABLE ROW LEVEL SECURITY;
ALTER TABLE item_copies FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS item_copies_org_isolation ON item_copies;
CREATE POLICY item_copies_org_isolation ON item_copies
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE inventory_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE inventory_sessions FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS inventory_sessions_org_isolation ON inventory_sessions;
CREATE POLICY inventory_sessions_org_isolation ON inventory_sessions
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE inventory_scans ENABLE ROW LEVEL SECURITY;
ALTER TABLE inventory_scans FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS inventory_scans_org_isolation ON inventory_scans;
CREATE POLICY inventory_scans_org_isolation ON inventory_scans
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE circulation_policies ENABLE ROW LEVEL SECURITY;
ALTER TABLE circulation_policies FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS circulation_policies_org_isolation ON circulation_policies;
CREATE POLICY circulation_policies_org_isolation ON circulation_policies
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE loans ENABLE ROW LEVEL SECURITY;
ALTER TABLE loans FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS loans_org_isolation ON loans;
CREATE POLICY loans_org_isolation ON loans
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE holds ENABLE ROW LEVEL SECURITY;
ALTER TABLE holds FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS holds_org_isolation ON holds;
CREATE POLICY holds_org_isolation ON holds
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());
...
ALTER TABLE authority_terms ENABLE ROW LEVEL SECURITY;
ALTER TABLE authority_terms FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS authority_terms_org_isolation ON authority_terms;
CREATE POLICY authority_terms_org_isolation ON authority_terms
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE authority_term_relations ENABLE ROW LEVEL SECURITY;
ALTER TABLE authority_term_relations FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS authority_term_relations_org_isolation ON authority_term_relations;
CREATE POLICY authority_term_relations_org_isolation ON authority_term_relations
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE bibliographic_subject_terms ENABLE ROW LEVEL SECURITY;
ALTER TABLE bibliographic_subject_terms FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_subject_terms_org_isolation ON bibliographic_subject_terms;
CREATE POLICY bibliographic_subject_terms_org_isolation ON bibliographic_subject_terms
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE bibliographic_name_terms ENABLE ROW LEVEL SECURITY;
ALTER TABLE bibliographic_name_terms FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_name_terms_org_isolation ON bibliographic_name_terms;
CREATE POLICY bibliographic_name_terms_org_isolation ON bibliographic_name_terms
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE bibliographic_geographic_terms ENABLE ROW LEVEL SECURITY;
ALTER TABLE bibliographic_geographic_terms FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_geographic_terms_org_isolation ON bibliographic_geographic_terms;
CREATE POLICY bibliographic_geographic_terms_org_isolation ON bibliographic_geographic_terms
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

ALTER TABLE bibliographic_genre_terms ENABLE ROW LEVEL SECURITY;
ALTER TABLE bibliographic_genre_terms FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS bibliographic_genre_terms_org_isolation ON bibliographic_genre_terms;
CREATE POLICY bibliographic_genre_terms_org_isolation ON bibliographic_genre_terms
  USING (organization_id = app_current_org_id())
  WITH CHECK (organization_id = app_current_org_id());

-- 2) user_credentials：沒有 organization_id，因此用 users 做 EXISTS 限制
ALTER TABLE user_credentials ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_credentials FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS user_credentials_org_isolation ON user_credentials;
CREATE POLICY user_credentials_org_isolation ON user_credentials
  USING (
    EXISTS (
      ...
      FROM users u
      WHERE u.id = user_id
        AND u.organization_id = app_current_org_id()
    )
  )
  WITH CHECK (
    EXISTS (
